...
import { getStatusLabel, placementPolygon, pointString,   rotatedPoints, rotatedSize } from '../../lib/project';
import { edgeMarkersForPart } from '../edgeProfiles';
import {
...
function edgeProfileSvg(
  ...
  return markers.map((marker) => {
    ...
    return [
      ...
      text(labelX, labelY, shortLabel, 10, '#1f5f87', 700, 'middle'),
    ].join('');
  }).join('');
}
...
function textureItemSvg(project: Project, item: TextureItem, scale: number, offsetX: number, offsetY: number, index: number) {
  ...
  const detail = project.details.find((item) => item.id === part.detailId);
  const edgeProfiles = edgeProfileSvg(project, part, detail, layout.rotation, scale);
