...
describe('occupiedEdgeSides', () => {
  ...
  /*
   * ЗМІНА КОНТРАКТУ — СВІДОМА (Б-002, 07.09.2026, рішення власника).
   * Раніше ребро Г-зарізу (`BC_lcut1`) зайнятим не вважалось, бо панель
   * кромок його взагалі не показувала. Тепер це повноцінна сторона:
   * нога на ній закриває торець так само, як на звичайній. Дуга
   * (`B_radius`) поводиться як і раніше.
   */
  it('друге доповнення на ребрі підписане #2; дуга не рахується, ребро Г-зарізу — рахується', () => {
    const out = occupiedEdgeSides({ subDetails: { 'fold_A#2': {}, fold_B_radius: {}, leg_BC_lcut1: {} } });
    expect(out).toEqual({ A: 'Потовщення (A #2)', BC_lcut1: 'Нога (BC_lcut1)' });
  });

  it('підпис зайнятого ребра Г-зарізу йде показуваним ім\'ям із sideLabels', () => {
    const out = occupiedEdgeSides({
      subDetails: { leg_BC_lcut1: {} },
      sideLabels: { BC_lcut1: 'C1' },
    });
    expect(out).toEqual({ BC_lcut1: 'Нога (C1)' });
  });
