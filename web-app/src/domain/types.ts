...
export interface ElementDefinition {
  ...
  attachInset?: number;
  /**
   * №172 — СТИК ІЗ БАТЬКОМ ПРЯМИЙ, а не вус 45°.
   *
   * Не вводиться руками: ставить `engines/attachmentShrink` — коли
   * доповнення втоплене або коли на цій стороні стоїть кромка. Правило
   * власника 09.09: кромка річ видима, отже вона диктує примикання, і
   * закривати її вусом не можна. Читають і план стиків, і кошторис.
   */
...
export interface ProductSinkDef {
  ...
  depth: number;
  /**
   * №156: поворот чаші навколо власного центру, ГРАДУСИ, проти годинникової
   * стрілки. Отвір у стільниці повертається разом із нею — обидва читають
   * одне поле (див. `sinkCutout`).
   */
  rotation?: number;
  /**
...
export interface Point {
  ...
  sideId?: string;
  /**
   * Ребро, що ПОЧИНАЄТЬСЯ в цій точці, — різ стику, а не сторона виробу
   * (03.09.2026). Відсутність імені для цього недостатня: без імені лишаються
   * й проміжні точки дуг, а метраж кромки бере половину «кутового переходу»
   * до сусідньої сторони — і на розрізаній деталі дотягувався по різу.
   */
  cut?: boolean;
}
...
export interface SurfaceCutout {
  ...
  edgeProcessing?: string;
  /**
   * №156: поворот вирізу навколо власного центру, ГРАДУСИ, проти годинникової
   * стрілки (0 або відсутнє — як було, сторонами по осях деталі). Для кола
   * значення не має сенсу і не читається.
   */
  rotation?: number;
}
...
export interface ManualJoint {
  ...
  jointType?: string;
  /**
   * ПАРА СТОРІН, МІЖ ЯКИМИ СТОЇТЬ СТИК (№141, 08.09.2026).
   *
   * Стик, поставлений на кресленні, належить конкретному полю: «між H і F»,
   * тобто в лівому виступі П-подібної. Без цієї пари рушій знав лише
   * координату лінії — і різав КОЖЕН шматок, крізь який вона проходить, тобто
   * заодно й праву ногу. Пара робить різ адресним: один стик — один шов.
   *
   * Порожньо — старі стики (панель «+ Вертикальний», омега/лямбда): для них
   * поведінка лишається наскрізною, як була.
   */
  sideId?: string;
  oppositeSideId?: string;
  /** Сторона, від якої користувач міряв відступ (для підпису, не для різу). */
