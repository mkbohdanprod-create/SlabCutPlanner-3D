          ...
          <section className="pdf-section quote-settings quote-edge-settings">
            ...
            {edgeProfiles.map((profile) => (
              <label key={profile.id}>
                {profile.label}
                <input type="number" value={settings.edgePrices[profile.id as EdgeProfileType] ?? 0} onChange={(e) => patchEdgePrice(profile.id as EdgeProfileType, numberValue(Number(e.target.value)))} />
              </label>
            ))}
          </section>
