...
export function QuotePanel() {
  ...
  /**
   * Клієнтський PDF друкується ЗАВЖДИ (рішення власника 03.09.2026).
   *
   * Історія. 26.08 друк без контрагента заборонили: без нього 1С віддає
   * ЗАГАЛЬНИЙ прайс, і менеджер міг прийняти його за ціну свого клієнта.
   * Логіка була правильна, але на практиці замикала роботу: поки довідник
   * контрагентів недоступний або клієнт іще не заведений, менеджер не міг
   * роздрукувати навіть порожній бланк — ні показати склад замовлення, ні
   * взяти на зустріч.
   *
   * Рішення власника: **друкувати, навіть якщо контрагента і цін немає —
   * хай друкує з нулями**. Нуль у документі чесніший за відсутній
   * документ: видно склад робіт, видно, що ціни ще не проставлені, і
   * видно, що це не остаточна пропозиція.
   *
   * Що лишається на своєму місці: підпис «за загальним прайсом» у шапці
   * таблиці (`generalPrices` нижче) — саме він і попереджає, що ціни не
   * персональні. Він не залежить від цієї зміни.
   */
  /**
        ...
        <div className="quote-head bg-white rounded-lg shadow-sm border border-slate-200 p-6 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title={!doc.contragentId
                ? 'Контрагента не вибрано — документ надрукується за загальним прайсом (ціни без відповіді 1С будуть нульові)'
