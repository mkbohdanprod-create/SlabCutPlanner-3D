...
import {     TemplateInput, TemplateSide, TemplateCheck} from './SvgComponents';
  ...
  return (
    <div className="schema reference-schema u-reference u-template-shell">
      <div className="u-template" aria-label="П-подібна схема розмірів">
        ...
        <TemplateCheck x={520} y={238} label="Стик вертикальний Ω" checked={detail.jointOmegaDirection === 'vertical'} onChange={(checked) => updateDetail({ jointOmegaDirection: checked ? 'vertical' : undefined })} />
        <TemplateCheck x={520} y={266} label="Стик горизонтальний Ω" checked={detail.jointOmegaDirection === 'horizontal'} onChange={(checked) => updateDetail({ jointOmegaDirection: checked ? 'horizontal' : undefined })} />
        <TemplateCheck x={520} y={294} label="Стик вертикальний λ" checked={detail.jointLambdaDirection === 'vertical'} onChange={(checked) => updateDetail({ jointLambdaDirection: checked ? 'vertical' : undefined })} />
        <TemplateCheck x={520} y={322} label="Стик горизонтальний λ" checked={detail.jointLambdaDirection === 'horizontal'} onChange={(checked) => updateDetail({ jointLambdaDirection: checked ? 'horizontal' : undefined })} />
      </div>
    </div>
  );
}