...
// Одна реалізація на весь проєкт — у `engines/geometryUtils`.
export { textureGroupKey as assemblyGroupKey } from '../../engines/geometryUtils';
...
export function findAngledSideSnap(part: DetailPart, placement: Placement, slab: SlabInstance, placements: Placement[], parts: DetailPart[], includeAligned = false) {
  ...
  const otherEdges = placements.flatMap((other) => {
    ...
    if (!otherPart) return [];
    const segments = polygonSegments(placementPolygon(otherPart, other), `placement:${other.id}`);
    
    // Extend segments for collinear snapping if sawOvercut is defined
    const sawOvercut = 2000; // virtual axis extension (could use project.referenceData?.serviceParams?.sawOvercut if passed, but typically we want infinite or large extension for snapping)
    return segments.map(seg => {
      const dx = seg.end.x - seg.start.x;
      const dy = seg.end.y - seg.start.y;
      const len = Math.hypot(dx, dy);
      if (len < 0.1) return seg;
      const dirX = dx / len;
      const dirY = dy / len;
      return {
        ...seg,
        start: { x: seg.start.x - dirX * sawOvercut, y: seg.start.y - dirY * sawOvercut },
        end: { x: seg.end.x + dirX * sawOvercut, y: seg.end.y + dirY * sawOvercut },
        length: seg.length + sawOvercut * 2
      };
    });
  });
