...
export function BacklightRig({ on, onSwap }: {
  ...
  /** Перезбирає списки світла й матеріалів — на початку кожного переходу. */
  const collect = () => {
    lights.current = [];
    stones.current = [];
    scene.traverse((object) => {
      ...
      if (light.isLight) {
        lights.current.push({ light, base: light.intensity });
        return;
      }
      ...
      list.forEach((material) => {
        ...
        if (mat && (mat as { isMeshStandardMaterial?: boolean }).isMeshStandardMaterial && mat.map) {
          stones.current.push({ mat, baseEnv: mat.envMapIntensity ?? 1 });
        }
      });
    });
