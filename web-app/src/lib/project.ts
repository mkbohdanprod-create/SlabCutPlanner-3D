...
export function rotatedLocalPoints(points: Point[], rotation: Rotation, width: number, height: number, referencePoints = points, allowances?: {top?: number, bottom?: number, left?: number, right?: number}): Point[] {
  let expandedPoints = points;
  let expandedRefPoints = referencePoints;
  
  if (allowances) {
    const expand = (p: Point) => {
      let nx = p.x;
      let ny = p.y;
      if (Math.abs(p.x - 0) < 0.1 && allowances.left) nx -= allowances.left;
      if (Math.abs(p.x - width) < 0.1 && allowances.right) nx += allowances.right;
      if (Math.abs(p.y - 0) < 0.1 && allowances.top) ny -= allowances.top;
      if (Math.abs(p.y - height) < 0.1 && allowances.bottom) ny += allowances.bottom;
      return { x: nx, y: ny };
    };
    expandedPoints = points.map(expand);
    expandedRefPoints = referencePoints.map(expand);
  }

  const rotatedReference = expandedRefPoints.map((point) => rotatePoint(point, rotation, width, height));
...
export function placementPolygon(part: DetailPart, placement: Placement): Point[] {
  return translatePoints(rotatedPoints(part, placement.rotation, placement), placement.x, placement.y);
}
