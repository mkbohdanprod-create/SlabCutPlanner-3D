...
export function extractProductionFacts(
  ...
  walkElements(project.products).forEach(({ productId, path, element }) => {
    (element.joints ?? []).forEach((joint, index) => {
      /*
       * КЛЮЧ ДЕДУПЛІКАЦІЇ — З АДРЕСОЮ (03.09.2026).
       *
       * Було `joint.id`, і цього виявилось замало: редактор дає стикам ім'я
       * від СЛОТА (`joint_wall_panel_A`), а слот унікальний лише всередині
       * виробу. Два різні вироби зі стіновою панеллю на стороні A діставали
       * той самий id — і другий стик мовчки випадав із фактів. На кейсі
       * 81-1750325 це 1.821 м склейки і один пропил, які цех робить, а
       * компанія не виставляє.
       *
       * Дедуплікація потрібна для іншого випадку — коли ОДИН стик записаний
       * на ОБОХ елементах, які він з'єднує (так буває в дереві виробу, і на
       * це є свій тест). Тому ключ — у межах ВИРОБУ, а не глобальний і не
       * до окремого елемента:
       *   · той самий стик на двох елементах одного виробу → один факт;
       *   · різні стики з однаковим іменем у різних виробах → два факти.
       * Усередині одного виробу імена стиків унікальні: вони походять зі
       * слотів доповнень, а слот на виріб один.
       */
      const key = `${productId}|${joint.id ?? `${path}#${index}`}`;
