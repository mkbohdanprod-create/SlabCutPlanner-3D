...
// Ключ текстурної групи живе в `geometryUtils` — один на рушій, канвас і снапінг.
...
function tryPlaceMainThenLooseElements(
  ...
  const mainParts = group.filter((part) => part.isMain && !isAttachedElement(part, group));
  const missingMainParts = mainParts.filter((part) => !context.placedIds.has(part.id));
  const elements = group.filter((part) => isAttachedElement(part, group) && !context.placedIds.has(part.id));
...
/**
 * Сторона батька, до якої кріпиться парт.
 *
 * Два шляхи дають її по-різному: legacy-підворот несе `edgeSide`, підворот
 * виробу — `parentDetailSide` (він головний парт свого Елемента, тому
 * `edgeSide` у нього порожній).
 */
export function attachedSideOf(part: DetailPart) {
  return part.edgeSide ?? part.parentDetailSide;
}

/**
 * Парт, який кріпиться до іншого парта тієї ж групи.
 *
 * Раніше ознакою було просто `!isMain`, і цього вистачало, поки підворот
 * генерувався з властивостей деталі. У виробі підворот — окремий Елемент,
 * отже головний парт СВОЄЇ деталі; для пакувальника він виглядав самостійною
 * деталлю, і розгортка до нього не застосовувалась. Саме тому доповнення
 * лягали купкою збоку, а не по сторонах стільниці.
 */
export function isAttachedElement(part: DetailPart, group: DetailPart[]) {
  if (!part.isMain) return true;
  if (!part.parentDetailId || !attachedSideOf(part)) return false;
  return group.some((other) => other !== part && other.detailId === part.parentDetailId);
}
...
function attachedPlacement(
  ...
  return attachedPlacementToSide(main, mainPart, part, group, attachedSideOf(part), main.rotation, clearance);
}

/**
 * Розгортка доповнення виробу: до потрібної сторони і з потрібним поворотом.
 *
 * Поворот не вгадуємо — обираємо той, у якому довжина парта вздовж сторони
 * найближча до довжини самої сторони. Підворот 600×100 на вертикальній стороні
 * заввишки 600 має стати вертикальною смужкою, а не лягти впоперек.
 */
function productAttachmentPlacements(
  main: Placement,
  mainPart: DetailPart,
  part: DetailPart,
  group: DetailPart[],
  ...
  const side = attachedSideOf(part);
  if (!side) return [];
  const segment = sideSegment(mainPart, side, main.rotation);
  if (!segment) return [];
  ...
  return [0, 90, 180, 270]
    .map((delta) => (((main.rotation + delta) % 360) + 360) % 360 as Rotation)
    .map((rotation) => {
      const size = rotatedSize(part, rotation);
      const alongSide = horizontal ? size.width : size.height;
      return { rotation, score: Math.abs(alongSide - segmentLength) };
    })
    .sort((a, b) => a.score - b.score)
    .map(({ rotation }) => attachedPlacementToSide(main, mainPart, part, group, side, rotation, clearance))
...
function placementCandidates(
  ...
  // Доповнення виробу (підворот/потовщення як окремий Елемент): розгортаємо
  // до своєї сторони, перебираючи повороти від найдоречнішого.
  if (part.isMain && part.parentDetailSide && !part.edgeSide) {
    const unfolded = productAttachmentPlacements(main, mainPart, part, group, clearance);
    if (unfolded.length) return uniquePlacements([...unfolded, ...(primary ? [primary] : [])]);
  }
...
function placeFullTextureElements(
  ...
  // Доповнення виробу шукає батька за detailId: у нього власний parentLabel,
  // бо це окремий Елемент, а не парт, згенерований із властивостей деталі.
  const mainByDetailId = new Map(mainPlacements.map((item) => [item.part.detailId, item]));
  const elements = group
    .filter((part) => isAttachedElement(part, group) && !alreadyPlacedIds.has(part.id))
    .sort((a, b) => (
  ...
  for (let elementIndex = 0; elementIndex < elements.length; elementIndex += 1) {
    ...
    const main = mainByLabel.get(element.parentLabel)
      ?? (element.parentDetailId ? mainByDetailId.get(element.parentDetailId) : undefined);
...
function tryPlaceFullTextureGroup(project: Project, group: DetailPart[], context: PackingContext, stepDivisor: number) {
  // Прикріплені доповнення сюди не потрапляють: їх розкладає
  // `placeFullTextureElements` відносно батька, а не за textureOffset.
  const mainParts = group.filter((part) => part.isMain && !isAttachedElement(part, group));
  const clearance = packingClearance(project);
...
function autoPackFullTexture(project: Project, parts: DetailPart[], context: PackingContext) {
  ...
  groups.forEach((group) => {
    const hasElements = group.some((part) => isAttachedElement(part, group));
    const hasMultipleMainParts = group.filter((part) => part.isMain && !isAttachedElement(part, group)).length > 1;
...
export function detectConflicts(project: Project, parts: DetailPart[], placements: Placement[]): Placement[] {
  return placements.map((placement) => {
    ...
    const holes = holesForPlacement(part, placement);
    const sawOvercut = project.referenceData?.serviceParams?.sawOvercut ?? 0;
    const conflict = placements.some((other) => {
      ...
      const otherPolygon = polygonForPlacement(otherPart, other);
      const otherHoles = holesForPlacement(otherPart, other);

      if (physicalPolygonsOverlap(polygon, holes, otherPolygon, otherHoles)) {
        return true;
      }
      ...
      if (sawOvercut > 0) {
        for (let i = 0; i < polygon.length; i++) {
          ...
          const dirX = dx / len;
          const dirY = dy / len;
          const s1 = { x: p1.x - dirX * sawOvercut, y: p1.y - dirY * sawOvercut };
          const s2 = { x: p2.x + dirX * sawOvercut, y: p2.y + dirY * sawOvercut };
          
          for (let j = 0; j < otherPolygon.length; j++) {
            const op1 = otherPolygon[j];
            const op2 = otherPolygon[(j + 1) % otherPolygon.length];
            if (segmentsIntersectStrict(s1, s2, op1, op2)) {
              return true;
            }
          }
        }
