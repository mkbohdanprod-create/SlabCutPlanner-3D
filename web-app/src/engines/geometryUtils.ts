...
export function sideVertexIndices(
  ...
  // ШМАТОК ПІСЛЯ РІЗУ СТИКОМ (03.09.2026, скарга фокус-групи).
  //
  // Якщо в парта Є мапа сторін, але потрібної сторони в ній НЕМАЄ — це
  // означає рівно одне: цієї сторони на цьому шматку не існує, її зрізав
  // стик. Раніше код падав нижче, у позиційну таблицю «сторона = ребро за
  // номером», і чесне «немає» перетворювалось на впевнену брехню: підпис
  // кромки лягав на ребро з тим самим порядковим номером — тобто саме на
  // лінію різу. На Г-подібній, розрізаній стиками на три шматки, це
  // давало «r2_top» на стиках і водночас зникнення позначок на реальних
  // зовнішніх сторонах (сторін E/F у таблиці для чотирьох вершин просто
  // немає, тому вони мовчки губились).
  //
  // Позиційна таблиця лишається для партів БЕЗ мапи сторін — це цілі
  // прості деталі, де сторона справді дорівнює ребру за угодою імен.
