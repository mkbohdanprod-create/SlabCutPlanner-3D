...
const newExtractServices = `export function extractServices(
  ...
  let totalAreaSqM = 0;
  const processElement = (element: ProductElement, isRoot: boolean) => {
    ...
    // Manual overrides from customServices (legacy extraction into manualOverrides)
    if (def.customServices) {
      def.customServices.forEach((cs) => {
        // We push this into manualOverrides so it's applied correctly later
    ...
    if (element.joints) {
      element.joints.forEach(joint => {
        ...
        const slotA = joint.a.elementRef;
        const slotB = joint.b.elementRef;
        ...
        if (joint.type === 'miter45') {
          if (catalog.CUT_45) addReq(catalog.CUT_45, lengthM * 2, pairKey, \`joint:\${joint.id}\`);
          if (catalog.GLUING_45) addReq(catalog.GLUING_45, lengthM, pairKey, \`joint:\${joint.id}\`);
        } else if (joint.type === 'glued') {
          if (catalog.GLUING_STRAIGHT) addReq(catalog.GLUING_STRAIGHT, lengthM, pairKey, \`joint:\${joint.id}\`);
        }
      });
    }

    if (element.additions) {
      element.additions.forEach(add => processElement(add, false));
    }
  };

  if (project.products) {
    project.products.forEach(prod => {
      if (prod.elements) prod.elements.forEach(el => processElement(el, true));
    });
  }
        ...
        if (sideLengthM > 0) {
          ...
          const processProfile = (pId: string | undefined, isTop: boolean) => {
            ...
            if (edgeProfileDef && edgeProfileDef.operations && edgeProfileDef.operations.length > 0) {
              ...
              if (project.projectMaterial === 'acryl' || project.projectMaterial === 'compact') {
          ...
          if (project.projectMaterial !== 'acryl' && project.projectMaterial !== 'compact' && catalog.CUT_STRAIGHT) {
  ...
  if (project.projectMaterial) {
    ...
    if (project.projectMaterial === 'ceramic') matService = catalog.MATERIAL_CERAMIC;
    if (project.projectMaterial === 'natural') matService = catalog.MATERIAL_NATURAL;
    if (project.projectMaterial === 'acrylic') matService = catalog.MATERIAL_ACRYLIC;
  ...
  return applyOverrides(deduped, manualOverrides);
}
`;

const startIndex = code.indexOf('export function extractServices');
const endIndex = code.indexOf('export function calculateServices');

if (startIndex !== -1 && endIndex !== -1) {
  code = code.substring(0, startIndex) + newExtractServices + code.substring(endIndex);
  fs.writeFileSync('src/engines/servicesExtractor.ts', code);
  console.log("Replaced extractServices successfully!");
} else {
